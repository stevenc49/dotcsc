...
int8_t nrk_TCB_init (nrk_task_type *Task, NRK_STK *ptos, NRK_STK *pbos, uint16_t stk_size, void *pext, uint16_t opt)
{
	...
	//Init CBS Task Variables
    if(Task->Type==CBS_TASK)
	{
    	nrk_task_TCB[Task->task_ID].isCBSTask=true;
    	nrk_task_TCB[Task->task_ID].maximumBudget=_nrk_time_to_ticks(Task->cpu_reserve);
    	nrk_task_TCB[Task->task_ID].budget=nrk_task_TCB[Task->task_ID].maximumBudget;
    	nrk_task_TCB[Task->task_ID].exceededPeriod=false;

	}
    else nrk_task_TCB[Task->task_ID].isCBSTask=false;
  ...
  nrk_task_TCB[Task->task_ID].task_plvl = Task->plvl;
	
 //nrk_task_TCB[Task->task_ID].task_preempt_level = Task->preempt_level;
//printf("NRK TEST: taskID: %d   pl: %d    preempt_level: %d \r\n", Task->task_ID, nrk_task_TCB[Task->task_ID].preempt_level, Task->preempt_level); 
  	//	double count = 0;
	...
	if( Task->Type == IDLE_TASK){
		...
		Task->plvl = 0;
		nrk_task_TCB[Task->task_ID].task_plvl = 0;	         
}
printf("NRK TEST: taskID: %d   pl: %d    preempt_level: %d \r\n", Task->task_ID, nrk_task_TCB[Task->task_ID].task_plvl, Task->plvl);
...
void _nrk_timer_tick(void)
{
	...
	//nrk_task_TCB[Task->task_ID].absolute_deadline += nrk_task_TCB[Task->task_ID].period;

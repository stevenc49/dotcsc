...
class CalendarExample:
  ...
  def _DateRangeQuery(self, start_date='2009-04-01', end_date='2009-05-01'):
...
#      list_events.append(an_event)
  ...
  def _GetAndDeleteEvent(self, dict_events={}):
    """Search for and delete event based on full text searching """

    dict_events = self._DateRangeQuery()

    print 'Enter the number of the event you wish to delete'
    input = int(raw_input('> '))
    self.cal_client.DeleteEvent( dict_events[input].GetEditLink().href )
  ...
  def Run(self):
    ...
    try:
      while True:
        ...
        if choice == 1:
          self._InsertQuickAddEvent()
        elif choice == 2:
          self._DateRangeQuery()
        elif choice == 3:
          self._GetAndDeleteEvent()
        elif choice == 7:
          return
